...
function Task() {
    ...
    //     if (!isAddTaskModalOpen || isRenderChange) {
            useEffect(() => {
                if (projectId) {
                    axios.get(`http://localhost:9000/api/project/${projectId}`)
    .then((res) => {
        const tasks = res.data.task[0]; // Ensure this is correct
        console.log("All tasks:", tasks);

        const requestedTasks = tasks.filter(task => task.stage === "Requested").sort((a, b) => a.order - b.order);
        const todoTasks = tasks.filter(task => task.stage === "To do").sort((a, b) => a.order - b.order);
        const inProgressTasks = tasks.filter(task => task.stage === "In Progress").sort((a, b) => a.order - b.order);
        const doneTasks = tasks.filter(task => task.stage === "Done").sort((a, b) => a.order - b.order);

        console.log("Requested tasks:", requestedTasks);
    ...
    .catch((error) => {
        toast.error('Something went wrong');
            ...
            useEffect(() => {
                // Debugging: Print columns to verify that they are being set correctly
    ...
    const updateTodo = (data) => {
        axios.put(`http://localhost:9000/api/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`http://localhost:9000/api/project/${projectId}/task/${taskId}`)
            .then((res) => {
