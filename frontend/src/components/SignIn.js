...
const SignIn = ({ setIsAuthenticated }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:9000/api/signin', { email, password });
            setIsAuthenticated(true);
            navigate('/'); // Redirect to main layout page
        } catch (err) {
    ...
    return (
        <div style={containerStyle}>
            <h1 style={headingStyle}>Sprint Board</h1>
            <form onSubmit={handleSubmit} style={formStyle}>
                <h2 style={formHeadingStyle}>Sign In</h2>
            ...
            <div style={contactUsStyle}>
                ...
                <p style={contactTextStyle}>For support, email us at:</p>
                <a href="mailto:support@example.com" style={contactEmailStyle}>sprint@example.com</a>
                <p style={contactTextStyle}>Or call us at:</p>
                <p style={contactPhoneStyle}>98237423828</p>
            </div>
        </div>
    );
};
